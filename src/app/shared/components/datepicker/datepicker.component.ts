...
import { UtilService } from './services/datepicker.util.service';
...
export class DatePicker implements OnChanges, ControlValueAccessor {
  ...
  selectedDate: IDate = {year: 0, month: 0, day: 0};
  ...
  onUserDateInput(value: string): void {
    ...
    if (value.length === 0) {
      ...
      let date: IDate = this.utilService.isDateValid(value, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableDays, this.opts.disableDateRanges, this.opts.monthLabels, this.opts.enableDays);
  ...
  updateDateValue(date: IDate, clear: boolean): void {
    // Updates date values
    ...
    for (let i = 1; i < 7; i++) {
      ...
      if (i === 1) {
        ...
        for (let j = 1; j < 8; j++) {
          ...
          let date: IDate = {year: y, month: cmo === this.currMonthId ? m : m + 1, day: dayNbr};
          week.push({
            dateObj: date, cmo: cmo, currDay: this.isCurrDay(dayNbr, m, y, cmo, today),
            dayNbr: this.utilService.getDayNumber(date),
            disabled: this.utilService.isDisabledDay(date, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableDays, this.opts.disableDateRanges, this.opts.enableDays),
