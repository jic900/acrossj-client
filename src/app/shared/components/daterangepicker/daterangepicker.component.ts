...
export class DateRangePicker implements OnChanges, ControlValueAccessor {
  ...
  beginDate: IDate = {year: 0, month: 0, day: 0};
  endDate: IDate = {year: 0, month: 0, day: 0};
  preventBefore: IDate = {year: 0, month: 0, day: 0};
  preventAfter: IDate = {year: 0, month: 0, day: 0};
  titleAreaTextBegin: string = '';
  titleAreaTextEnd: string = '';

  // Default options
  opts: IOptions = {
    dayLabels: <IDayLabels> {su: 'Sun', mo: 'Mon', tu: 'Tue', we: 'Wed', th: 'Thu', fr: 'Fri', sa: 'Sat'},
  ...
  onUserDateRangeInput(value: string): void {
    ...
    if (this.invalidDateRange) {
      this.inputFieldChanged.emit({
        ...
        valid: !(value.length === 0 || this.invalidDateRange)
      });
    }
  }
  ...
  selectBeginDate(date: IDate): void {
    this.beginDate = date;
  ...
  getDateRangeModel(beginDate: IDate, endDate: IDate): IDateRangeModel {
  ...
  getPreviousDate(date: IDate): IDate {
    ...
    for (let i = 1; i < 7; i++) {
      ...
      if (i === 1) {
        ...
        for (let j = pm; j <= dInPrevM; j++) {
          let date: IDate = {year: m === 1 ? y - 1 : y, month: m === 1 ? 12 : m - 1, day: j};
          week.push({
            dateObj: date,
            cmo: cmo,
            currDay: this.isCurrDay(j, m, y, cmo, today),
            dayNbr: this.getDayNumber(date),
            disabled: this.drus.isDisabledDay(date, this.opts.disableUntil, this.opts.disableSince, this.opts.disableDates, this.opts.disableDateRanges, this.preventBefore, this.preventAfter, this.opts.enableDates),
            range: false
          });
        }
  ...
  setHeaderBtnDisabledState(m: number, y: number): void {
    ...
    if (this.opts.disableHeaderButtons) {
      ...
      dpy = this.drus.isMonthDisabledByDisableUntil({
        year: y - 1,
        month: m,
